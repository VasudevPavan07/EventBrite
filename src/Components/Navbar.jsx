...
const Navbar = ({ toggleRegistrationForm,toggleLoginForm }) => {
const[isDropdownOpen ,setIsDropdownOpen] = useState(false);
const[selectedMenuItem ,setSelectedItem] = useState(null);
...
const handleMenuItemClick =(menuItem)=>{
  ...
  return (
    <div className='h-14 bg-slate-200 flex items-center justify-between'>
        <div className='flex'>
      <svg viewBox="0 0 200 36" className='p-3  w-36 fill-orange-500 cursor-pointer'>
    <g fillRule="evenodd">
      <g>
  ...
  <input className='h-10 p-3 m-2 rounded-md cursor-pointer' placeholder='search for events' type="text" />
  </div>
  <div className=''>
<ul className="flex flex-row  list-none font-semibold cursor-pointer">
    <li className="mr-12 p-3 rounded-3xl hover:bg-slate-100">Browse Events</li>
    <li className="mr-12 p-3 rounded-3xl hover:bg-slate-100"> <Link to="/host-event">Host an Event</Link></li>
    <li className="mr-12 p-3 rounded-3xl hover:bg-slate-100" onClick={toggleDropdown}><span className='flex  items-center '>Help   <img  src="/arrow-down.png" alt="Dropdown-Icon" className='ml-3 w-3 h-3' /></span>
  
    {
      isDropdownOpen &&( <ul className='absolute p-2 mt-4 bg-slate-50 '><li className="mr-12 p-3  hover:bg-slate-200 " onClick={() => handleMenuItemClick('Help Center')}>Help Center</li>
      <li className="mr-12 p-3   hover:bg-slate-200 " onClick={() => handleMenuItemClick('Find Your Tickets')}>Find Your Tickets</li>
      <li className="mr-12 p-3  hover:bg-slate-200" onClick={() => handleMenuItemClick('Contact Your Event Organizer')}>Contact Your Event Organizer</li></ul>)
    }
    </li>
    <li className="mr-12 p-3 rounded-3xl hover:bg-slate-100" onClick={toggleLoginForm}>Log In</li>
    <li className="mr-12 p-3 rounded-3xl hover:bg-slate-100" onClick={toggleRegistrationForm}>Sign Up</li>
</ul>
</div>
<div>
